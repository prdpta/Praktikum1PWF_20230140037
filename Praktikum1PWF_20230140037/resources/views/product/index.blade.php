                    ...
                    <tbody class="text-gray-300 divide-y divide-gray-700">
                        @forelse ($products as $product)
                            <tr class="hover:bg-gray-700/30 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 font-bold text-white uppercase">{{ $product->name }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-center items-center gap-4">
                                        <a href="{{ route('product.show', $product->id) }}" class="text-gray-400 hover:text-blue-400 transition" title="View Detail">
                                        ...
                                        @can('update', $product)
                                        <a href="{{ route('product.edit', $product->id) }}" class="text-gray-400 hover:text-yellow-500 transition" title="Edit Product">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        @can('delete', $product)
                                        <form action="{{ route('product.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Yakin hapus produk ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-gray-400 hover:text-red-500 transition" title="Delete Product">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
