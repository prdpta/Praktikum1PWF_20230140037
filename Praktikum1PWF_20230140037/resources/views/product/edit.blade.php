                ...
                <form action="{{ route('product.update', $product->id) }}" method="POST" class="space-y-6" novalidate>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $product->name) }}" 
                               class="w-full bg-gray-900 border {{ $errors->has('name') ? 'border-red-500' : 'border-gray-700' }} text-white rounded-xl">
                        @error('name')
                    ...
                    <div class="flex items-center gap-4 pt-4 border-t border-gray-700">
                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-2 rounded-xl font-bold">UPDATE PRODUCT</button>
